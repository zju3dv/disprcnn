...
def disparityregression(x, maxdisp, mindisp=0):
    assert x.shape[1] == int(maxdisp - mindisp)
    bsz, disp_range, H, W = x.shape
    disp = torch.arange(mindisp, maxdisp).reshape((1, disp_range, 1, 1)).float().to(x.device)
...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.branch1 = nn.Sequential(nn.AvgPool2d((56, 56), stride=(56, 56)),
