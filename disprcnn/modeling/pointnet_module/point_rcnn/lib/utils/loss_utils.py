...
def _sigmoid_cross_entropy_with_logits(logits, labels):
    ...
    loss = loss + torch.log1p(torch.exp(-torch.abs(logits)))
...
def get_reg_loss(pred_reg, reg_label, loc_scope, loc_bin_size, num_head_bin, anchor_size,
                 ...
                 loc_y_scope=0.5, loc_y_bin_size=0.25, get_ry_fine=False,
                 loss_mask=None):
    """
    ...
    :return:
    """
    loss_reduction_method = 'none' if loss_mask is not None else 'mean'
    ...
    loss_x_bin = F.cross_entropy(pred_reg[:, x_bin_l: x_bin_r], x_bin_label,
                                 reduction=loss_reduction_method)
    loss_z_bin = F.cross_entropy(pred_reg[:, z_bin_l: z_bin_r], z_bin_label,
                                 reduction=loss_reduction_method)
    if loss_mask is not None:
        loss_x_bin = (loss_x_bin * loss_mask.float()).sum()
        loss_z_bin = (loss_z_bin * loss_mask.float()).sum()
        if loss_mask.sum() != 0:
            loss_x_bin = loss_x_bin / loss_mask.sum().float()
            loss_z_bin = loss_z_bin / loss_mask.sum().float()
    reg_loss_dict['loss_x_bin'] = loss_x_bin.item()
    reg_loss_dict['loss_z_bin'] = loss_z_bin.item()
    loc_loss = loc_loss + loss_x_bin + loss_z_bin
    ...
    if get_xz_fine:
        ...
        loss_z_res = F.smooth_l1_loss((pred_reg[:, z_res_l: z_res_r] * z_bin_onehot).sum(dim=1), z_res_norm_label,
                                      reduction=loss_reduction_method)
        if loss_mask is not None:
            loss_x_res = (loss_x_res * loss_mask.float()).sum()
            loss_z_res = (loss_z_res * loss_mask.float()).sum()
            if loss_mask.sum() != 0:
                loss_x_res = loss_x_res / loss_mask.sum().float()
                loss_z_res = loss_z_res / loss_mask.sum().float()
        reg_loss_dict['loss_x_res'] = loss_x_res.item()
        reg_loss_dict['loss_z_res'] = loss_z_res.item()
        loc_loss = loc_loss + loss_x_res + loss_z_res
    ...
    if get_y_by_bin:
        ...
        loc_loss = loc_loss + loss_y_bin + loss_y_res
    else:
        y_offset_l, y_offset_r = start_offset, start_offset + 1
        start_offset = y_offset_r

        loss_y_offset = F.smooth_l1_loss(pred_reg[:, y_offset_l: y_offset_r].sum(dim=1),
                                         y_offset_label,
                                         reduction=loss_reduction_method)
        if loss_mask is not None:
            loss_y_offset = (loss_y_offset * loss_mask.float()).sum()
            if loss_mask.sum() != 0:
                loss_y_offset = loss_y_offset / loss_mask.sum().float()
        reg_loss_dict['loss_y_offset'] = loss_y_offset.item()
        loc_loss = loc_loss + loss_y_offset
    ...
    size_loss = F.smooth_l1_loss(size_res_norm, size_res_norm_label,
                                 reduction=loss_reduction_method)
    if loss_mask is not None:
        size_loss = (size_loss * loss_mask.float()[:, None]).sum()
        if loss_mask.sum() != 0:
            size_loss = size_loss / loss_mask.float().sum()
