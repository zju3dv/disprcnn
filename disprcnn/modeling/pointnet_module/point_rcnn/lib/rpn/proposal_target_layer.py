...
from ..utils.roipool3d import roipool3d_utils as roipool3d_utils
from ..utils.iou3d import iou3d_utils as iou3d_utils
...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, input_dict, gt_boxes3d):
        ...
        reg_valid_mask = ((batch_roi_iou > self.cfg.RCNN.REG_FG_THRESH) & valid_mask).long()

        # classification label
        batch_cls_label = (batch_roi_iou > self.cfg.RCNN.CLS_FG_THRESH).long()
        invalid_mask = (batch_roi_iou > self.cfg.RCNN.CLS_BG_THRESH) & (batch_roi_iou < self.cfg.RCNN.CLS_FG_THRESH)
        batch_cls_label[valid_mask == 0] = -1
        batch_cls_label[invalid_mask > 0] = -1

        output_dict = {'sampled_pts': sampled_pts.view(-1, self.cfg.RCNN.NUM_POINTS, 3),
                       'pts_feature': sampled_features.view(-1, self.cfg.RCNN.NUM_POINTS, sampled_features.shape[3]),
        ...
        # torch.cuda.synchronize()
        # augtimer.toc()
        # print('aug', augtimer.average_time)
        return output_dict
    ...
    def sample_rois_for_rcnn(self, roi_boxes3d, gt_boxes3d):
        ...
        for idx in range(batch_size):
            ...
            fg_thresh = min(self.cfg.RCNN.REG_FG_THRESH, self.cfg.RCNN.CLS_FG_THRESH)
            ...
            easy_bg_inds = torch.nonzero((max_overlaps < self.cfg.RCNN.CLS_BG_THRESH_LO)).view(-1)
            ...
            if fg_num_rois > 0 and bg_num_rois > 0:
                ...
                rand_num = torch.randperm(fg_num_rois).type_as(gt_boxes3d).long()
            ...
            if fg_rois_per_this_image > 0:
                ...
                fg_rois, fg_iou3d = self.aug_roi_by_noise_torch(fg_rois_src, gt_of_fg_rois, iou3d_src,
                                                                aug_times=self.cfg.RCNN.ROI_FG_AUG_TIMES)
            ...
            if bg_rois_per_this_image > 0:
                ...
                aug_times = 1 if self.cfg.RCNN.ROI_FG_AUG_TIMES > 0 else 0
        ...
        # torch.cuda.synchronize()
        # sampletimer.toc()
        # print('sample', sampletimer.average_time)
    ...
    def aug_roi_by_noise_torch(self, roi_boxes3d, gt_boxes3d, iou3d_src, aug_times=10):
        ...
        pos_thresh = min(self.cfg.RCNN.REG_FG_THRESH, self.cfg.RCNN.CLS_FG_THRESH)
    ...
    def random_aug_box3d(self, box3d):
        ...
        """
        if self.cfg.RCNN.REG_AUG_METHOD == 'single':
            ...
            return aug_box3d
        elif self.cfg.RCNN.REG_AUG_METHOD == 'multiple':
            ...
            aug_box3d = torch.cat([box3d[0:3] + pos_shift, box3d[3:6] * hwl_scale, box3d[6:7] + angle_rot], dim=0)
            return aug_box3d
        elif self.cfg.RCNN.REG_AUG_METHOD == 'normal':
    ...
    def data_augmentation(self, pts, rois, gt_of_rois):
        ...
        angles = (torch.rand((batch_size, boxes_num), device=pts.device) - 0.5 / 0.5) * (np.pi / self.cfg.AUG_ROT_RANGE)
