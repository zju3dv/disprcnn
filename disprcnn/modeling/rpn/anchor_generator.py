...
class AnchorGenerator(nn.Module):
    ...
    def __init__(
        ...
        if len(feature_strides) == 1:
            feature_stride = feature_strides[0]
            cell_anchors = [
                generate_anchors(feature_stride, scales, ratios).float()
            ]
        else:
            if len(feature_strides) != len(scales):
                raise RuntimeError("FPN should have #anchor_strides == #sizes")

            cell_anchors = []
            for feature_stride, scale in zip(feature_strides, scales):
                cell_anchors.append(generate_anchors(
                    feature_stride,
                    scale if isinstance(scale, (tuple, list)) else (scale,),
                    ratios
                ).float())
    ...
    def forward(self, image_list, feature_maps):
        feature_shapes = [list(feature_map.shape[-2:]) for feature_map in feature_maps]
        anchors_over_all_feature_maps = generate_anchors_all_pyramids(self.scales, self.ratios, feature_shapes,
                                                                      self.feature_strides, 1)
        anchors = []
...
def generate_anchors(
        feature_stride=16, scales=(32, 64, 128, 256, 512), ratios=(0.5, 1, 2)
):
    ...
    return _generate_anchors(
        feature_stride,
        np.array(scales, dtype=np.float) / feature_stride,
        # np.array(scales, dtype=np.float),
        np.array(ratios, dtype=np.float),
    )
