...
import torch
import numpy as np
from disprcnn.layers.misc import interpolate
...
class BinaryMaskList(object):
    ...
    def resize(self, size, use_max_pooling=False):
        try:
            iter(size)
        except TypeError:
        ...
        if width < 0 or height < 0:
            ...
            return BinaryMaskList(self.masks, self.size)
        assert width > 0
        assert height > 0
        ...
        if not use_max_pooling:
            ...
            resized_masks = F.adaptive_max_pool2d(
                self.masks[None].float(),
                (height, width))[0].type_as(self.masks)
...
class PolygonList(object):
    ...
    def resize(self, size, use_max_pooling=False):
        if invalid_size(size):
            return PolygonList(self.polygons, self.size)
        resized_polygons = []
        for polygon in self.polygons:
...
class SegmentationMask(object):
    ...
    def __init__(self, instances, size, mode="poly"):
        ...
        assert isinstance(size, (list, tuple)), f'expect {size} to be list or tuple.'
        assert len(size) == 2
    ...
    def resize(self, size, use_max_pooling=False):
        if invalid_size(size):
            return SegmentationMask(self.instances, self.size, self.mode)
        resized_instances = self.instances.resize(size, use_max_pooling)
        resized_size = size
...
def invalid_size(size):
    return (isinstance(size, int) and size < 0) or \
           (isinstance(size, (tuple, list)) and any(a < 0 for a in size))
